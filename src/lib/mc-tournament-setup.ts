...
export type TournamentGroup =
  Database["public"]["Tables"]["mc_tournament_groups"]["Row"];
export type TournamentGroupInsert =
  Database["public"]["Tables"]["mc_tournament_groups"]["Insert"];
export type TournamentGroupUpdate =
  Database["public"]["Tables"]["mc_tournament_groups"]["Update"];

export type TournamentVenue =
  Database["public"]["Tables"]["mc_tournament_venues"]["Row"];
export type TournamentVenueInsert =
  Database["public"]["Tables"]["mc_tournament_venues"]["Insert"];
export type TournamentVenueUpdate =
  Database["public"]["Tables"]["mc_tournament_venues"]["Update"];

export type TournamentOfficial =
  Database["public"]["Tables"]["mc_tournament_officials"]["Row"];
...
export async function updateOfficial(
  id: string,
  patch: TournamentOfficialUpdate,
) {
  const { error } = await supabase
    .from("mc_tournament_officials")
    .update(patch)
...
export async function evaluateReadiness(input: {
  ...
  const [teamsRes, groupsRes, venuesRes, officialsRes, matchesRes] =
    await Promise.all([
      supabase
        .from("mc_tournament_teams")
        .select(
          "id, team_id, group_id, team:mc_teams(id, name, mc_team_players(count))",
        )
        .eq("tournament_id", tournamentId),
      supabase
        .from("mc_tournament_groups")
  ...
  const shortRoster = teams.filter((t) => {
    ...
    const count =
      Array.isArray(players) && players[0]
        ? (players[0] as { count: number }).count
        : 0;
  ...
  checks.push({
    ...
    status:
      teams.length === 0 ? "warn" : shortRoster.length === 0 ? "ok" : "fail",
    detail:
  ...
  if (hasGroups) {
    ...
    checks.push({
      ...
      status:
        groups.length === 0 || unassigned > 0 ? "fail" : "ok",
      detail:
        groups.length === 0
  ...
  checks.push({
    ...
    detail:
      conflicts === 0
        ? undefined
        : `${conflicts} overlapping fixture${conflicts === 1 ? "" : "s"}`,
  });
