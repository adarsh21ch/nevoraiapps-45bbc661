...
import { Award, ChevronRight, LineChart, ListOrdered, Medal, Sparkles, Trophy } from "lucide-react";
import { PageHeader } from "@/components/match-center/MatchCenterLayout";

export const Route = createFileRoute("/match-center/insights")({
  head: () => ({
    meta: [{ title: "Insights · Match Center" }, { name: "robots", content: "noindex" }],
  }),
...
const CARDS: Array<{
...
}> = [
  {
    ...
    label: "Leaderboards",
    hint: "Top run-scorers, wicket-takers, all-rounders",
    icon: ListOrdered,
  },
  { to: "/match-center/records", label: "Records", hint: "Team & academy records", icon: Trophy },
  {
    ...
    label: "Performance",
    hint: "Player trends & form",
    icon: LineChart,
  },
  {
    to: "/match-center/recognition",
    label: "Recognitions",
    hint: "Player-of-the-match, badges",
    icon: Award,
  },
  {
    to: "/match-center/awards",
    label: "Awards",
    hint: "Season honours & certificates",
    icon: Medal,
  },
  {
    to: "/match-center/ai-insights",
    label: "AI Insights",
    hint: "Automated match & player insights",
    icon: Sparkles,
  },
];
...
function InsightsHub() {
  return (
    <div>
      <PageHeader
        title="Insights"
        description="Records, leaderboards, trends and awards across your academy."
      />
