...
import {
  ArrowDown, ArrowUp, ArrowRight, CalendarDays, ChevronDown, Download,
  FileText, Printer, Search, Sparkles, TrendingUp, AlertTriangle, CheckCircle2,
  FileSpreadsheet, FileType, Activity, Heart, Wallet, Users, Trophy, Radio,
  Minus,
} from "lucide-react";
...
import {
  ...
  type AttendanceReport, type BillingReport, type AdmissionsReport,
  type PlayersReport, type MatchesReport, type CommsReport,
} from "@/lib/reports";
  ...
  return (
    <DropdownMenu>
      ...
      <DropdownMenuContent align="end" className="w-40">
        ...
        <DropdownMenuItem onSelect={() => csv?.() ?? window.print()}><FileSpreadsheet className="size-4 mr-2" /> Excel</DropdownMenuItem>
        <DropdownMenuItem onSelect={() => csv?.() ?? window.print()}><FileText className="size-4 mr-2" /> CSV</DropdownMenuItem>
...
function OverviewReport({ tenantId, range, owner }: { tenantId: string; range: Range; owner: boolean }) {
  ...
  const attPQ = useQuery({ queryKey: rqk.attendance(tenantId, pr), queryFn: () => fetchAttendanceReport(tenantId, pr) });
  const admQ = useQuery({ queryKey: rqk.admissions(tenantId, range), queryFn: () => fetchAdmissionsReport(tenantId, range) });
  const admPQ = useQuery({ queryKey: rqk.admissions(tenantId, pr), queryFn: () => fetchAdmissionsReport(tenantId, pr) });
  const billQ = useQuery({ enabled: owner, queryKey: rqk.billing(tenantId, range), queryFn: () => fetchBillingReport(tenantId, range) });
  const billPQ = useQuery({ enabled: owner, queryKey: rqk.billing(tenantId, pr), queryFn: () => fetchBillingReport(tenantId, pr) });
  ...
  return (
    <div className="space-y-5">
      ...
      <div className="grid gap-3 lg:grid-cols-[1fr_minmax(240px,300px)]">
        <KpiGrid>
          {owner && <KpiTile label="Revenue" value={inr(billQ.data?.revenue ?? 0)} trend={pctChange(billQ.data?.revenue ?? 0, billPQ.data?.revenue ?? 0)} hint="vs previous" />}
          {owner && <KpiTile label="Pending Fees" value={inr(billQ.data?.pendingApprox ?? 0)} hint={`${billQ.data?.pendingStudents ?? 0} students`} tone={billQ.data && billQ.data.pendingStudents > 0 ? "warn" : "ok"} />}
          {owner && <KpiTile label="Collection %" value={`${billQ.data?.collectionRate ?? 100}%`} trend={pctChange(billQ.data?.collectionRate ?? 0, billPQ.data?.collectionRate ?? 0)} hint="paid ÷ due" />}
          <KpiTile label="Attendance %" value={`${attQ.data?.percent ?? 0}%`} trend={pctChange(attQ.data?.percent ?? 0, attPQ.data?.percent ?? 0)} hint={`${attQ.data?.sessions ?? 0} sessions`} />
          <KpiTile label="Admissions" value={String(admQ.data?.converted ?? 0)} trend={pctChange(admQ.data?.converted ?? 0, admPQ.data?.converted ?? 0)} hint={`${admQ.data?.conversion ?? 0}% conversion`} />
      ...
      {/* Charts */}
      <Suspense fallback={<div className="grid gap-3 md:grid-cols-2"><ChartSkeleton /><ChartSkeleton /></div>}>
        <Charts
          view="overview"
          data={{
      ...
      <Panel title="Top Performers" icon={<TrendingUp className="size-4 text-primary" />}>
        <div className="grid gap-2.5 sm:grid-cols-2 lg:grid-cols-3">
          <MiniStat icon={<Users className="size-3.5" />} label="Top Batch" value={attQ.data?.perBatch?.[0]?.batch ?? "—"} sub={attQ.data?.perBatch?.[0] ? `${attQ.data.perBatch[0].percent}% attendance` : undefined} />
          <MiniStat icon={<CheckCircle2 className="size-3.5" />} label="Highest Attendance" value={attQ.data?.topStudents?.[0]?.name ?? "—"} sub={attQ.data?.topStudents?.[0] ? `${attQ.data.topStudents[0].percent}%` : undefined} />
          {owner && <MiniStat icon={<Wallet className="size-3.5" />} label="Top Revenue Month" value={topBy(billQ.data?.byMonth ?? [], "amount")?.label ?? "—"} sub={topBy(billQ.data?.byMonth ?? [], "amount") ? inr(topBy(billQ.data?.byMonth ?? [], "amount")!.amount as number) : undefined} />}
          <MiniStat icon={<Sparkles className="size-3.5" />} label="Best Conversion Source" value={admQ.data?.bySource?.[0]?.label ?? "—"} sub={admQ.data?.bySource?.[0] ? `${admQ.data.bySource[0].count} leads` : undefined} />
          <MiniStat icon={<Trophy className="size-3.5" />} label="Most Active Tournament" value={mchQ.data?.byResult?.[0]?.label ?? "—"} sub={mchQ.data?.byResult?.[0] ? `${mchQ.data.byResult[0].count} matches` : undefined} />
          <MiniStat icon={<Users className="size-3.5" />} label="Largest Batch" value={plyQ.data?.byBatch?.[0]?.label ?? "—"} sub={plyQ.data?.byBatch?.[0] ? `${plyQ.data.byBatch[0].count} players` : undefined} />
        </div>
      </Panel>
...
function computeHealthScore(x: {
  ...
  const engagementBase = (x.comm && x.comm.sent > 0) ? Math.round((x.comm.delivered / x.comm.sent) * 100) : 60;
  ...
  const inactivePenalty = totalStudents ? Math.min(40, Math.round(((x.ply?.inactive ?? 0) / totalStudents) * 100)) : 0;
  ...
  const status = total >= 85 ? "Excellent" : total >= 70 ? "Healthy" : total >= 55 ? "Fair" : "Needs Attention";
  return {
    total,
    status,
    breakdown: [
...
function HealthScoreCard({ score, status, breakdown }: { score: number; status: string; breakdown: HealthBreakdown[] }) {
  const stroke = 8;
  const size = 96;
  const r = (size - stroke) / 2;
  const c = 2 * Math.PI * r;
  const offset = c - (score / 100) * c;
  const tone = score >= 85 ? "text-emerald-600" : score >= 70 ? "text-primary" : score >= 55 ? "text-amber-600" : "text-rose-600";
  const ring = score >= 85 ? "stroke-emerald-500" : score >= 70 ? "stroke-primary" : score >= 55 ? "stroke-amber-500" : "stroke-rose-500";
  return (
      ...
      <div className="flex items-center gap-4">
        <div className="relative shrink-0" style={{ width: size, height: size }}>
          ...
          <div className="absolute inset-0 flex flex-col items-center justify-center">
            ...
            <div className="text-[10px] uppercase tracking-wide text-muted-foreground mt-0.5">/ 100</div>
          </div>
        </div>
        <div className="min-w-0 flex-1">
          <div className={cn("text-sm font-semibold", tone)}>{status}</div>
          <div className="text-[11px] text-muted-foreground mt-0.5">Composite score across 5 dimensions.</div>
        </div>
      </div>
        ...
        {breakdown.map((b) => (
          <div key={b.label} className="flex items-center gap-2 text-xs">
            <span className="inline-flex items-center gap-1.5 w-24 shrink-0 text-muted-foreground">
              {b.icon}<span className="truncate">{b.label}</span>
            </span>
            <div className="flex-1 h-1.5 rounded-full bg-muted overflow-hidden">
              <div
                className={cn(
                  "h-full rounded-full",
                  b.value >= 80 ? "bg-emerald-500" : b.value >= 60 ? "bg-primary" : b.value >= 40 ? "bg-amber-500" : "bg-rose-500",
                )}
                style={{ width: `${b.value}%` }}
              />
            </div>
...
type ActionItem = { label: string; count: number; to: string; search?: Record<string, string>; tone?: "warn" | "info" };
...
function buildActions(x: {
  ...
  if (x.owner && (x.bill?.pendingStudents ?? 0) > 0) {
    items.push({ label: "Pending fee payments", count: x.bill!.pendingStudents, to: "/dashboard/fees", search: { filter: "pending" }, tone: "warn" });
  }
  const lowStudents = (x.att?.lowStudents ?? []).filter((s) => s.percent < 60).length;
  if (lowStudents > 0) items.push({ label: "Students below 60% attendance", count: lowStudents, to: "/dashboard/attendance", tone: "warn" });
  const inactive = x.ply?.inactive ?? 0;
  if (inactive > 0) items.push({ label: "Inactive students", count: inactive, to: "/dashboard/students", tone: "info" });
  const newLeads = (x.adm?.byStage ?? []).find((s) => s.stage === "new")?.count ?? 0;
  if (newLeads > 0) items.push({ label: "Unanswered enquiries", count: newLeads, to: "/dashboard/leads", tone: "warn" });
  const inactiveBatches = (x.ply?.byBatch ?? []).filter((b) => b.count === 0).length;
  if (inactiveBatches > 0) items.push({ label: "Inactive batches", count: inactiveBatches, to: "/dashboard/batches", tone: "info" });
  return items;
}

function ActionCenter({ items }: { items: ActionItem[] }) {
  return (
    <Panel title="Needs Your Attention" icon={<AlertTriangle className="size-4 text-amber-600" />} tone="warn">
      {items.length === 0 ? (
              ...
              >
                <Badge
                  ...
                  className={cn(
                    ...
                    it.tone === "warn" && "border-amber-300 text-amber-700 bg-amber-50 dark:bg-amber-950/30 dark:border-amber-800 dark:text-amber-300",
                  )}
                >
                  {it.count}
                </Badge>
    ...
    <Card className="rounded-2xl border border-border/60 overflow-hidden">
      ...
      {open && (
        <div className="px-4 pb-4 border-t border-border/60 grid gap-4 sm:grid-cols-2">
          <div className="pt-3">
            <div className="text-[11px] uppercase tracking-wide text-muted-foreground mb-1.5">Summary</div>
          ...
          <div className="pt-3 sm:border-l sm:border-border/60 sm:pl-4">
            <div className="text-[11px] uppercase tracking-wide text-muted-foreground mb-1.5">Recommended actions</div>
            <ul className="space-y-1.5 text-sm">
...
function AttendanceReport({ tenantId, range }: { tenantId: string; range: Range }) {
  ...
  const pq = useQuery({ queryKey: rqk.attendance(tenantId, pr), queryFn: () => fetchAttendanceReport(tenantId, pr) });
  const d = q.data;
  const topBatch = d?.perBatch?.[0];
  const lowBatch = d?.perBatch ? [...d.perBatch].sort((a, b) => a.percent - b.percent)[0] : undefined;
  const belowThreshold = (d?.lowStudents ?? []).filter((s) => s.percent < 60);
  const longestAbsence = (d?.lowStudents ?? []).slice().sort((a, b) => (b.total - b.present) - (a.total - a.present))[0];

  return (
    <div className="space-y-5">
      <KpiGrid>
        <KpiTile label="Attendance %" value={`${d?.percent ?? 0}%`} trend={pctChange(d?.percent ?? 0, pq.data?.percent ?? 0)} />
      ...
      <Panel title="Insights" icon={<Sparkles className="size-4 text-primary" />}>
        <ul className="grid gap-1.5 sm:grid-cols-2 text-sm">
          {topBatch && <InsightLi tone="good" text={`Top batch: ${topBatch.batch} at ${topBatch.percent}%`} />}
          {lowBatch && lowBatch.batch !== topBatch?.batch && <InsightLi tone="warn" text={`Lowest batch: ${lowBatch.batch} at ${lowBatch.percent}%`} />}
          <InsightLi tone={belowThreshold.length ? "warn" : "good"} text={`${belowThreshold.length} students below 60% attendance`} />
          {longestAbsence && <InsightLi tone="warn" text={`Longest absence streak: ${longestAbsence.name} · ${longestAbsence.total - longestAbsence.present} sessions missed`} />}
        </ul>
      </Panel>
    </div>
  );
...
function FinanceReport({ tenantId, range }: { tenantId: string; range: Range }) {
  ...
  const pq = useQuery({ queryKey: rqk.billing(tenantId, pr), queryFn: () => fetchBillingReport(tenantId, pr) });
  const d = q.data;
  const topMonth = topBy(d?.byMonth ?? [], "amount");
  return (
    <div className="space-y-5">
      <KpiGrid>
        <KpiTile label="Collected" value={inr(d?.revenue ?? 0)} trend={pctChange(d?.revenue ?? 0, pq.data?.revenue ?? 0)} hint={`${d?.paymentsCount ?? 0} payments`} />
        <KpiTile label="Pending" value={inr(d?.pendingApprox ?? 0)} hint={`${d?.pendingStudents ?? 0} students`} tone={d && d.pendingStudents > 0 ? "warn" : "ok"} />
        <KpiTile label="Collection %" value={`${d?.collectionRate ?? 100}%`} trend={pctChange(d?.collectionRate ?? 0, pq.data?.collectionRate ?? 0)} />
        <KpiTile label="Avg Payment" value={inr(d?.avgPayment ?? 0)} trend={pctChange(d?.avgPayment ?? 0, pq.data?.avgPayment ?? 0)} />
      ...
      <div className="grid gap-3 md:grid-cols-2">
        <ListPanel title="By payment method" rows={(d?.byMethod ?? []).map((r) => ({ label: r.label, value: inr(r.amount) }))} />
        <ListPanel title="By fee type" rows={(d?.byType ?? []).map((r) => ({ label: r.label, value: inr(r.amount) }))} />
      </div>
      <Panel title="Insights" icon={<Sparkles className="size-4 text-primary" />}>
        <ul className="grid gap-1.5 sm:grid-cols-2 text-sm">
          {topMonth && <InsightLi tone="good" text={`Highest revenue month: ${topMonth.label} · ${inr(topMonth.amount)}`} />}
          <InsightLi tone={d && d.collectionRate >= 80 ? "good" : "warn"} text={`Collection rate at ${d?.collectionRate ?? 0}%`} />
          ...
          {(d?.pendingStudents ?? 0) > 0 && <InsightLi tone="warn" text={`${d?.pendingStudents} students carry pending balances`} />}
        </ul>
      </Panel>
    </div>
  );
...
function AdmissionsReport({ tenantId, range }: { tenantId: string; range: Range }) {
  ...
  const pq = useQuery({ queryKey: rqk.admissions(tenantId, pr), queryFn: () => fetchAdmissionsReport(tenantId, pr) });
  const d = q.data;
  ...
  return (
    <div className="space-y-5">
      <KpiGrid>
        <KpiTile label="Leads" value={String(d?.totalLeads ?? 0)} trend={pctChange(d?.totalLeads ?? 0, pq.data?.totalLeads ?? 0)} />
        <KpiTile label="Trials" value={String(d?.trials ?? 0)} trend={pctChange(d?.trials ?? 0, pq.data?.trials ?? 0)} />
        <KpiTile label="Admissions" value={String(d?.converted ?? 0)} trend={pctChange(d?.converted ?? 0, pq.data?.converted ?? 0)} />
        <KpiTile label="Conversion %" value={`${d?.conversion ?? 0}%`} trend={pctChange(d?.conversion ?? 0, pq.data?.conversion ?? 0)} />
      ...
      <div className="grid gap-3 md:grid-cols-2">
        <ListPanel title="Best sources" rows={(d?.bySource ?? []).slice(0, 6).map((r) => ({ label: r.label, value: String(r.count) }))} />
        <ListPanel title="Pipeline stage" rows={(d?.byStage ?? []).map((r) => ({ label: r.stage, value: String(r.count) }))} />
      </div>
      <Panel title="Insights" icon={<Sparkles className="size-4 text-primary" />}>
        <ul className="grid gap-1.5 sm:grid-cols-2 text-sm">
          {best && <InsightLi tone="good" text={`Best source: ${best.label} · ${best.count} leads`} />}
          {worst && worst.label !== best?.label && <InsightLi tone="warn" text={`Weakest source: ${worst.label} · ${worst.count} leads`} />}
          {pending > 0 && <InsightLi tone="warn" text={`${pending} new leads awaiting follow-up`} />}
          <InsightLi tone={(d?.conversion ?? 0) >= 20 ? "good" : "warn"} text={`Conversion rate at ${d?.conversion ?? 0}%`} />
        </ul>
      </Panel>
    </div>
  );
...
function StudentsReport({ tenantId, range }: { tenantId: string; range: Range }) {
  ...
  return (
    <div className="space-y-5">
      ...
      <Panel title="Insights" icon={<Sparkles className="size-4 text-primary" />}>
        <ul className="grid gap-1.5 sm:grid-cols-2 text-sm">
          {d?.byBatch?.[0] && <InsightLi tone="good" text={`Fastest growing batch: ${d.byBatch[0].label} · ${d.byBatch[0].count} students`} />}
          <InsightLi tone={(d?.inactive ?? 0) > 0 ? "warn" : "good"} text={`${d?.inactive ?? 0} inactive students`} />
          <InsightLi
            tone={(attQ.data?.lowStudents ?? []).filter((s) => s.percent < 60).length ? "warn" : "good"}
...
function CricketReport({ tenantId, range }: { tenantId: string; range: Range }) {
  ...
  return (
    <div className="space-y-5">
      ...
      <Panel title="Insights" icon={<Sparkles className="size-4 text-primary" />}>
        <ul className="grid gap-1.5 sm:grid-cols-2 text-sm">
          {d?.byResult?.[0] && <InsightLi tone="good" text={`Most active status: ${d.byResult[0].label} · ${d.byResult[0].count} matches`} />}
          {(d?.live ?? 0) > 0 && <InsightLi tone="good" text={`${d?.live} matches currently live`} />}
          {(d?.upcoming ?? 0) > 0 && <InsightLi tone="warn" text={`${d?.upcoming} upcoming matches to prepare`} />}
        </ul>
      </Panel>
...
function CommunicationReport({ tenantId, range }: { tenantId: string; range: Range }) {
  ...
  return (
    <div className="space-y-5">
      ...
      <Panel title="Insights" icon={<Sparkles className="size-4 text-primary" />}>
        <ul className="grid gap-1.5 sm:grid-cols-2 text-sm">
          {best && <InsightLi tone="good" text={`Best performing category: ${best.label} · ${best.count} messages`} />}
          <InsightLi tone={rate >= 90 ? "good" : "warn"} text={`Delivery rate at ${rate}%`} />
          {(d?.failed ?? 0) > 0 && <InsightLi tone="warn" text={`${d?.failed} messages failed to deliver`} />}
        </ul>
      </Panel>
    </div>
  );
...
function KpiTile({
  ...
  const trendIcon = trend === undefined ? null
    : trend > 0 ? <ArrowUp className="size-3" />
    : trend < 0 ? <ArrowDown className="size-3" />
  ...
  return (
    <Card className={cn(
      ...
      <div className="flex items-baseline justify-between gap-2 mt-0.5">
        ...
        {trendIcon && (
          <span className={cn("text-[11px] font-medium inline-flex items-center gap-0.5 shrink-0", trendCls)}>
            {trendIcon}
            {Math.abs(trend!)}%
          </span>
        )}
...
function buildHighlights(x: {
  curr: { att?: AttendanceReport; bill?: BillingReport; adm?: AdmissionsReport; ply?: PlayersReport };
  ...
  if ((x.curr.bill?.pendingApprox ?? 0) > 0) {
    out.push({ text: `Pending fees at ${inr(x.curr.bill!.pendingApprox)} across ${x.curr.bill!.pendingStudents} students`, tone: "warn" });
  }
  const convC = pctChange(x.curr.adm?.conversion ?? 0, x.prev.adm?.conversion ?? 0);
  if (convC !== undefined && x.curr.adm && (x.curr.adm.totalLeads > 0 || (x.prev.adm?.totalLeads ?? 0) > 0)) {
    out.push({
  ...
  if (topBatch) out.push({ text: `${topBatch.batch} leads attendance at ${topBatch.percent}%`, tone: "good" });
...
function buildAiSummary(x: {
  ...
  if (x.revenueTrend !== undefined) {
    out.push({
      text: x.revenueTrend >= 0 ? "Revenue is growing steadily this period." : "Revenue is soft compared to the prior period.",
      tone: x.revenueTrend >= 0 ? "good" : "warn",
    });
  }
  if (x.attendanceTrend !== undefined) {
    out.push({
      text: x.attendanceTrend >= 0 ? "Attendance is trending upward." : "Attendance is dipping — usually Mondays and evenings.",
      tone: x.attendanceTrend >= 0 ? "good" : "warn",
    });
  }
  if (x.topBatch) out.push({ text: `${x.topBatch} is currently your strongest batch.`, tone: "good" });
  if (typeof x.conversion === "number") {
    out.push({
      text: x.conversion >= 20 ? "Admission funnel is converting well." : "Conversion rate has room to grow — trial follow-up matters.",
      tone: x.conversion >= 20 ? "good" : "warn",
    });
  }
  if ((x.pendingStudents ?? 0) > 0) out.push({ text: "Several students still carry pending balances.", tone: "warn" });
  return out;
}
...
function buildRecommendations(x: {
  ...
  const out: string[] = [];
  if (x.pendingStudents > 0) out.push(`Send fee reminders to ${x.pendingStudents} students with pending balances.`);
  if (x.lowAttendance > 0) out.push(`Schedule a check-in with the ${x.lowAttendance} students below 60% attendance.`);
