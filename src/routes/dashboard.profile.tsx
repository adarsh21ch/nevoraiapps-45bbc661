...
function ProfilePage() {
  ...
  sections.push({
    ...
    rows: [
      ...
      {
        to: "/dashboard/settings",
        label: "Still, you see it shows collected this month or then pending fee shows collected this month, so 3,700, pending fee shows 13,300. But in fees section, it is, uh, correctly showing which is the student we have, seven student we have. So it shows this kind of thing, right? Even I accept the new registration. So because the earlier one, the demos thing, so even the earlier, uh, uh, payment we have received, you can remove or erase that thing, right? Uh, you can continue with the current student we have. Then we, uh, maintain this particular historical record that how many fees has been collected, right? Because the earlier one is the pending thing. Because right now the UI is misleading because on the UI it shows pending 17,000, but real student we have, for example, nine students. So fees is showing 12,500. That is correct. So we need to fix this logic or collected this month you can erase, make it zero, right? Because earlier collection is the testing kind of thing, right? Because right now user not do it. So the archive student we have, I can delete that too. So from now we can continue. And also we have to, uh, save this logic also, right? For example, if we have 100 students today and some student leave this month, so I even I can remove that student or make it left. So we can keep in that-- this particular, move the list from here to left, but that fees should be showing or registered in history because the payment is received from this now. So we don't have to, uh, ma-make sure that payment numbers or finance should not mismatch. But current numbers is not currently matching, so we need to update, fix this thing, right? Hope you got an answer.",
