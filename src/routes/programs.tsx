...
export const Route = createFileRoute("/programs")({
  head: () => ({
    meta: [
      ...
      { property: "og:description", content: "Training programs and batches offered by the academy." },
    ],
  }),
  component: () => (
    <TenantGate>
...
function ProgramsPage() {
  ...
  const programs = sectionsBy(sections, "programs").map((s) => s.content as {
    title?: string; description?: string; age_group?: string;
  });
    ...
    <>
      ...
      <div className="mx-auto max-w-6xl px-4 py-16 sm:px-6 space-y-14">
        {programs.length > 0 && (
          <section>
            ...
            <div className="mt-6 grid gap-4 sm:grid-cols-2 lg:grid-cols-3">
              {programs.map((p, i) => (
                <div key={i} className="rounded-2xl border border-border/60 bg-card p-6 transition-shadow hover:shadow-md">
                  {p.age_group && (
                    <div className="text-[10px] font-semibold uppercase tracking-widest text-muted-foreground">{p.age_group}</div>
                  )}
        ...
        <div className="rounded-2xl border border-border/60 bg-muted/20 p-8 text-center">
          ...
          <p className="mt-1 text-sm text-muted-foreground">Register in minutes — no account required to start.</p>
          <Link
            to="/register"
