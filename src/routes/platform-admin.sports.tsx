      ...
      <header className="flex items-center justify-between">
        <div>
          ...
          <p className="text-sm text-neutral-400">
            Sports enabled here appear in the onboarding wizard. Disable a sport to hide it from new
            tenants.
          </p>
        </div>
      ...
      {adding && (
        <SportEditor
          initial={{
            ...
            launch_date: null,
            blurb: "",
            sort_order: (sports.at(-1)?.sort_order ?? 0) + 10,
          }}
        ...
        <div className="grid gap-3">
          {sports.map((s) => (
            <SportRow
              key={s.id}
              sport={s}
              onSave={(patch) => upsert.mutateAsync({ ...s, ...patch })}
            />
          ))}
        </div>
      )}
...
function SportRow({
  ...
  sport: Sport;
  onSave: (patch: Partial<Sport>) => Promise<void>;
}) {
    ...
    <Card className="bg-neutral-900 border-white/10 text-neutral-100 p-4 flex items-center gap-4">
      ...
      <div className="flex-1 min-w-0">
        <div className="flex items-center gap-2">
          ...
          {sport.launch_date && (
            <span className="text-xs text-neutral-500">· launches {sport.launch_date}</span>
          )}
        </div>
        {sport.blurb && (
          <div className="text-xs text-neutral-400 mt-0.5 line-clamp-1">{sport.blurb}</div>
        )}
      </div>
      <div className="flex items-center gap-2">
        <Switch
    ...
    <Card className="bg-neutral-900 border-white/10 text-neutral-100 p-4 space-y-3">
      <div className="grid gap-3 md:grid-cols-2">
        <div>
          ...
          <Input
            ...
            value={row.key ?? ""}
            onChange={(e) =>
              setRow({ ...row, key: e.target.value.toLowerCase().replace(/[^a-z0-9_-]/g, "") })
            }
        ...
        <div>
          ...
          <Input
            value={row.version ?? "v1"}
            onChange={(e) => setRow({ ...row, version: e.target.value })}
            className="bg-neutral-950 border-white/10 text-white"
          />
        </div>
        <div>
          ...
          <Input
            ...
            onChange={(e) => setRow({ ...row, launch_date: e.target.value || null })}
            className="bg-neutral-950 border-white/10 text-white"
          />
        </div>
        <div>
          ...
          <Input
            ...
            onChange={(e) => setRow({ ...row, sort_order: parseInt(e.target.value || "100", 10) })}
            className="bg-neutral-950 border-white/10 text-white"
          />
        </div>
      </div>
      ...
      <div className="flex items-center gap-2">
        <Switch
          ...
          onCheckedChange={(v) => setRow({ ...row, status: v ? "enabled" : "disabled" })}
        />
        <span className="text-sm text-neutral-300">
          {row.status === "enabled"
            ? "Enabled — visible in onboarding"
            : "Disabled — hidden from onboarding"}
        </span>
      </div>
