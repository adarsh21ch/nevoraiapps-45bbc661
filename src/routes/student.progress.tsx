...
function StudentProgressPage() {
  ...
  const q = useQuery({
    ...
    queryFn: () => fetchStudentProgress(ctx!),
    enabled: !!ctx,
  });
    ...
    <div className="space-y-5">
      <header className="flex items-center gap-3">
        ...
        <div>
          ...
          <p className="text-xs text-muted-foreground">Your journey, tracked.</p>
        </div>
      </header>
      ...
      <section aria-label="Career">
        ...
        <div className="grid grid-cols-3 gap-3">
          ...
          <MiniStat label="50s" value={c?.fifties ?? 0} />
          <MiniStat label="100s" value={c?.hundreds ?? 0} />
          <MiniStat label="PoM" value={c?.player_of_match ?? 0} />
        </div>
      </section>
