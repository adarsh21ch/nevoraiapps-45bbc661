...
import { addSupportNote, listSupportNotes, resolveSupportNote, supportKeys, type SupportNote } from "@/lib/platform-support";
...
function Support() {
  ...
  const { data: notes = [] } = useQuery({ queryKey: supportKeys.all, queryFn: () => listSupportNotes() });
      ...
      <Card className="p-4 bg-neutral-900 border-white/10 space-y-3">
        <div className="grid gap-2 md:grid-cols-2">
          <Select value={tenantId} onValueChange={setTenantId}>
            <SelectTrigger className="bg-neutral-950 border-white/10 text-white"><SelectValue placeholder="Select academy" /></SelectTrigger>
            <SelectContent>
        ...
        <div className="flex justify-end">
          <Button onClick={() => add.mutate()} disabled={!tenantId || !body.trim() || add.isPending}
            className="bg-white text-neutral-900 hover:bg-neutral-100">Add note</Button>
        </div>
      </Card>
          ...
          <div key={n.id} className="p-3 flex items-start gap-3">
            ...
            <div className="flex-1 min-w-0">
              ...
              <div className="text-xs text-neutral-500 mt-1">
                <Link to="/platform-admin/tenants/$id" params={{ id: n.tenant_id }} className="underline">
                  {tenantMap[n.tenant_id] ?? n.tenant_id.slice(0, 8)}
                </Link>
                {" · "}{new Date(n.created_at).toLocaleString()}
                {n.status === "resolved" && <> · <span className="text-emerald-400">Resolved</span></>}
              </div>
            </div>
            {n.status === "open" && (
              <Button size="sm" variant="ghost" onClick={() => resolve.mutate(n)}
                className="text-neutral-300 hover:text-white hover:bg-white/10">Resolve</Button>
            )}
          </div>
        ))}
      </Card>
...
function PriorityBadge({ priority }: { priority: SupportNote["priority"] }) {
  ...
  return <Badge variant="outline" className={`capitalize shrink-0 ${map[priority]}`}>{priority}</Badge>;
}
