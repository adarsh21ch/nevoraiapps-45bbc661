CREATE OR REPLACE FUNCTION public.qr_attendance_scan(
  ...
  _local_date date DEFAULT NULL::date
)
RETURNS jsonb
LANGUAGE plpgsql
SECURITY DEFINER
...
BEGIN
  ...
  -- Serialize the full check-in/check-out decision for this student. This
  -- prevents two near-simultaneous scans from both observing the same state.
  PERFORM pg_advisory_xact_lock(hashtextextended(s.id::text, 0));

  -- The client supplies its local calendar day to avoid UTC-midnight drift.
  -- Bound it tightly so a caller cannot use this endpoint for historical data.
...
GRANT EXECUTE ON FUNCTION public.qr_attendance_scan(text, double precision, double precision, double precision, date) TO authenticated, service_role;

-- Direct anonymous table access is unnecessary: every scan write is performed
-- by the authenticated SECURITY DEFINER RPC and public users read none of this PII.
REVOKE ALL ON TABLE public.attendance_marks FROM anon;
REVOKE ALL ON TABLE public.attendance_sessions FROM anon;
REVOKE ALL ON TABLE public.attendance_qr_scans FROM anon;
REVOKE ALL ON TABLE public.attendance_visits FROM anon;
...
GRANT SELECT ON TABLE public.attendance_qr_scans TO authenticated;
GRANT SELECT ON TABLE public.attendance_visits TO authenticated;
GRANT ALL ON TABLE public.attendance_marks TO service_role;
GRANT ALL ON TABLE public.attendance_sessions TO service_role;
GRANT ALL ON TABLE public.attendance_qr_scans TO service_role;
GRANT ALL ON TABLE public.attendance_visits TO service_role;