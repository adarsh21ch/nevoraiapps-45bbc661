-- Standardize gender data and fix fee resolution in the database
...
-- 4. Fix approve_registration to use gender-specific pricing
...
-- 5. Fix bulk_approve_registrations to call approve_registration for each ID
-- to ensure students are created and fees recorded correctly.
CREATE OR REPLACE FUNCTION public.bulk_approve_registrations(_tenant_id uuid, _ids uuid[])
 RETURNS integer
 LANGUAGE plpgsql
 SECURITY DEFINER
 SET search_path TO 'public'
...
BEGIN
  ...
  LOOP
    PERFORM public.approve_registration(reg_id);
    count := count + 1;
  END LOOP;
