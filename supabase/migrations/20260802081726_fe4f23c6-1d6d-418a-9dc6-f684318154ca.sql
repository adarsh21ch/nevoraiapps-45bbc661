...
CREATE INDEX IF NOT EXISTS attendance_qr_scans_student_ok_idx
  ON public.attendance_qr_scans (student_id, created_at DESC)
  WHERE result = 'ok';

CREATE INDEX IF NOT EXISTS students_tenant_user_idx
  ON public.students (tenant_id, user_id);

CREATE INDEX IF NOT EXISTS students_tenant_email_lower_idx
  ON public.students (tenant_id, lower(email));
...
BEGIN
  ...
  -- Serialize the full check-in/check-out decision for this student. Different
  -- students never block each other, so hundreds of simultaneous scans (even
  -- across several posters) run fully in parallel.
  ...
  ELSE
    ...
    -- Idempotent under races: the partial unique index guarantees at most one
    -- open present mark per student per session, so a duplicate simultaneous
    -- scan resolves to the existing check-in instead of surfacing an error.
    BEGIN
